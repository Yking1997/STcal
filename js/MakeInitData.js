 const initMap = (function make_initMap() {
   ...
   for (let prop in baseMap) {
     ...
     _selTab ? _selTab.length = 0 : _selTab = [];
     ...
     if (baseMap[prop].exData) {
       _exData ? _exData.length = 0 : _exData = [];
       _exData[0] = 0;
       for (let i = 0, len = baseMap[prop].exData.length; i < len; i++) {
         _exData[i + 1] = copy(baseMap[prop].exData[i], true);
       }
       baseMap[prop].exData = copy(_exData, true);
       _exData ? _exData.length = 0 : _exData = [];
     }
   ...
   function delChildenNode(obj) {
     for (var a in obj) {
       if (typeof(obj[a]) == "object") {
         delChildenNode(obj[a]); //递归遍历
       } else {
         let _txt = String(obj[a]);
         delete newMap[_txt]; //如果是值就显示
       }
     }
   }
   delChildenNode(_baseChild, newMap);
   /**
 ...
 (function getopMapNumParCalRef() {
   let par = [],
     cal = [],
     ref = [];
   initMap.selSTCode.forEach(function(item) {
     if (item) {
       [
         [par, 'dataPar'],
         [ref, 'exDataPar'],
 ...
 class User {
   constructor(STCode, Parprop, Parvalue, Parunit, Calprop, Calvalue, Calunit, Refprop, Refvalue, Refunit) {
     this.STCode = STCode;
